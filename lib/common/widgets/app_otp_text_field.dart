...
class _AppOtpTextFieldState extends State<AppOtpTextField> {
  ...
  void changeFocusToNextNodeWhenValueIsEntered({
    ...
    print('$value   $indexOfTextField    $isDeleteKey');
    if (isDeleteKey && value.isEmpty) {
    ...
    if (value.length > 0 && value.length == widget.fieldLength) {
  ...
  void onSubmit({required List<String?> verificationCode}) {
    if (verificationCode.length ==
        (widget.fieldLength *
            widget.numberOfFields)) if (verificationCode
